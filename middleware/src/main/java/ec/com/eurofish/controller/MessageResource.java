...
public class MessageResource {
        ...
        public Uni<Response> genericMessage(MessageRequest request) {
                ...
                return Uni.createFrom()
                                ...
                                .onItem()
                                .transform(paas -> generic.request(request, GenericPaaSRequest.fromMongoItem(paas)))
        ...
        public Uni<Response> businessOneMessage(MessageRequest request) {
                ...
                // business.retrievePaaS(request.getDestination())
                // .bySerial(request.getDestination())
                // .subscribe()
                // .with(item -> future.complete(item)));
                return Uni.createFrom().item(business.bySerial(request.getDestination()))
                                .onItem().transform(paas -> business.request(request, paas))
                                // .item(business.retrievePaaS(request.getDestination()))
                                // .onItem().transform(paas -> business.request(request,
                                // BusinessOnePaaSRequest.fromMongoItem(paas)))
                                .onItem().transform(body -> {
                                        registry
