...
declare v_id integer;
		...
		v_body json;
begin
	select id, description, web_id, ip, port, root_path, timeout, business_one, body_as_header, body
	into v_id, v_desc, v_webid, v_ip, v_port, v_path, v_timeout, v_business, v_as_header, v_body
	...
	if exists (select 1 from paas where id = v_id) then
		...
		--insert
		insert into paas (description, webid, ip, port, root_path, timeout, body_as_header, body, business_one)
		values (v_desc, v_webid, v_ip, v_port, v_path, v_timeout, v_as_header, v_body, v_business) 
